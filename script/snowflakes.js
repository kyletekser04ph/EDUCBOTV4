...
module.exports. run = async function({ api, event, args }) {
  ...
  const cliff = await new Promise(resolve => { api.sendMessage('🔍 Searching Please Wait...', event.threadID, (err, info1) => {
    ...
    const response = await axios.get(`https://hazeyy-apis.onrender.com/snow?prompt=${encodeURIComponent(user)}`);
    const content = formatFont(response.data);
