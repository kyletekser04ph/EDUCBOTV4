...
const mentionID = Object.keys(event.mentions)[0] || (event.messageReply && event.messageReply.senderID);
    ...
    if (!mentionID) {
      return api.sendMessage('Please mention or reply a user you want to be a pet', event.threadID, event.messageID);
    }
...
/**  let userID;
    ...
    const response = `https://api-canvass.vercel.app/pet?userid=${mentionID}`;
