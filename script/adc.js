  ...
  if (url[0].indexOf('pastebin') !== -1) {
    axios.get(url[0]).then(i => {
      ...
      fs.writeFile(`${__dirname}/${args[0]}.js`, data, "utf-8", function(err) {
        ...
        api.sendMessage(`Applied the code to ./script/${args[0]}.js, successfull`, threadID, messageID);
      });
    });
  }
    ...
    request(options, function(error, response, body) {
      ...
      load('.language-js').each((index, el) => {
        ...
        fs.writeFile(`${__dirname}/${args[0]}.js`, code, "utf-8", function(err) {
          ...
          return api.sendMessage(`Added this code "${args[0]}.js", Successfull`, threadID, messageID);
        });
      });
    });
    return;
